...
const Comments = ({ postId }) => {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Errore durante l'invio del commento"
      );
    }
  };
  ...
  return (
    <div className="comments-section">
      <h2 className="mt-4 mb-4 form-label" style={{ fontSize: "20px" }}>
        Comments
      </h2>
      ...
      {user ? (
        ...
        <Alert variant="info">Log In to leave a comment</Alert>
      )}
      ...
      <ListGroup className="mt-4" style={{ borderRadius: "15px" }}>
        {comments.map((comment) => (
          <ListGroup.Item
            key={comment._id}
            className="comment-item form-container"
          >
            <div style={{ fontStyle: "italic" }} className="form-label">
              {comment.author?.firstName || "Utente Eliminato"}{" "}
              {comment.author?.lastName || ""}
            </div>
            <div className="comment-text" style={{ color: "white" }}>
              {comment.content}
